...
opening $w_n$ within the band width, $h_{\epsilon}$. \DM{Check band width $h_{\epsilon}$ subindex or superindex? or both. See eq. 18,19 and 20} Therefore, 
\begin{equation}
...
of a drop-weight impact test are compared with experimental ones as well as eigensoftening computations. \PNA{FALTA LO DE BUI}.

\subsection{Edge-cracked square panel in mode I}
\label{sec:3.1}

The aim of the problem presented hereinafter is to assess the capability of the \acrshort{lme}
approximants to improve the result \textit{versus} the standard linear interpolation. The application consists of a square plate of size $H = 1$ containing
an initial edge crack of length $0.25 \cdot H$ loaded in a pure mode I
by displacement control on the outer flanks of the plate (See Figure
...
over. They are due to the dynamic nature of the solver: \PNA{once the energy is release dinamically by the crack, the waves propagate along the domain.}
...
deformation gradient\MMP{CITA AQUI LOS QUE LO HACEN}. \textit{A priori} this would enhance the localisation properties of the proposed algorithms in more sophisticated domains.

Finally, for better capturing the measurement of forces made in the laboratory, more realistic contact tools have to be developed. Navas~\textif{et al.} demonstrate the importance of the stiffness of the support in order to obtain accurate results of the reaction forces. Interesting contact algorithms within the \acrshort{mpm} methodology are available in the literature \MMP{CITA ALGUNO Y SI SON CONSERVATIVOS EN ENERGIAS, MEJOR}. }
