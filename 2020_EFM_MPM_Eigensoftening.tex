...
(2012)\cite{Pandolfi_2012} for fracture of brittle materials.\DM{The evaluation of both approaches \cite{Navas_2017_ES} show that the eigenerosion algorithm significantly overestimates the tensile stress and the strain peaks, meanwhile eigensoftening captures the forces and crack
...
starting point at each time step. Therefore, each node is assigned to an
...
\DM{The proposed approach to overcome the two main shortcomings of EighenMPM \cite{Zhang_EE_2020} has been evaluated with ``two benchmark tests''.}The first one deals with the presence of stress instabilities provoked by the spatial discretization, appearing even
