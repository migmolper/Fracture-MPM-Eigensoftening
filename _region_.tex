...
(0,\infty)$.\MMP{Notice that $\beta$ has units of $[L]^{-2}$,
therefore it can be controlled by adjusting a dimensionless
parameter\footnote{To avoid confusion with the $\gamma$ parameter of
  the \acrshort{npc}, the dimensionless parameter defined in Arroyo \&
  Ortiz \cite{Arroyo2006} as $\gamma$ will be represent by
  $\widehat{\gamma}$ to preserve as much as possible the original notation.},
...
\end{document}
\message{ !name(2020_EFM_MPM_Eigensoftening.tex) !offset(-1131) }
