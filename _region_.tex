...
attained. For intermediate values a damage value between zero
...
\begin{equation}
  ...
  B_{\epsilon}(x_p^{k+1})} m_q \tens{\sigma}_{q,I}, 
\end{equation}
where $m_p$ is the total mass of the $\epsilon$-neighborhood, defined similarly than in the eigenerosion methodology (Eq.~\eqref{eq:mass-EE}).
%\begin{equation}
...
\acrshort{mpm} simulations is not required anymore. A linked
list of tributary nodes is reconstructed with the closest node as
starting point at each time step instead. Therefore, each node is assigned to an
...
\end{document}
\message{ !name(2020_EFM_MPM_Eigensoftening.tex) !offset(-1135) }
