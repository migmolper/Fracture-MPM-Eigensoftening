...
we will adopt a explicit predictor-corrector time integration
scheme. The purpose of this choice is motivated due its proved
...
\end{equation}
Consequently we can define an equivalent critical stress at the
...
introduced a band width $h_{\epsilon}$. The effective fracture strain
$\varepsilon_{\epsilon,f}$ is defined as the difference between the
strain at crack initialization, $\varepsilon_1(\vec{x}_p^{0})$, and
the current strain, $\varepsilon_1(\vec{x}_p^{k+1})$, for material
...
\end{document}
\message{ !name(2020_EFM_MPM_Eigensoftening.tex) !offset(-630) }
